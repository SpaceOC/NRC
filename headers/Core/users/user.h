	...
	*/
	...
	class User : public PrintableClass {
		...
		public:
			...
			User();
			// User Creation.
			User(const std::string& username, const UserPermissions& permissions, const std::string& language = "English", const std::string& password = "");
			User(User&);
			...
			void editPermissions(const UserPermissions& newPermissions);
			...
			VariableData getVar(std::string_view name);
