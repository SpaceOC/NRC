...
void core_experimental::runCode(const std::string& code, core::User* user) {
    try {
        ...
        std::string str;
        core_experimental::addFunctions(i, user, heap, str);
        try {
...
void core_experimental::runCode(const std::string& code, const core::Permissions& permissions) {
...
void core_experimental::runCode(const std::string& code, const core::Permissions& permissions, std::string& str) {
	core::User tempUser = core::User("UNKNOWN_TEMP_USER_FOR_RUNNING_COMMANDS_OR_JS_CODE", permissions);
